exports.handler = async (event) => {
  ...
  try {
    ...
    const enriched = enrichStaticData(symbol, market);
    return json({ ...enriched, budget });
  } catch (error) {
    const fallback = enrichStaticData(symbol, mockMarket(symbol));
    return json({
      ...fallback,
      budget,
      mode: "MOCK_FALLBACK",
...
async function getTaiwanMarket(symbol) {
  ...
  for (const ch of channels) {
    ...
    try {
      const response = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0", "Referer": "https://mis.twse.com.tw/stock/fibest.jsp" } });
      ...
      return {
        ...
        mode: "LIVE_TW",
        source: ch.source,
        notice: "已取得台股行情；基本面、法人、均線資料使用可擴充資料庫"
      };
    } catch (e) {}
  }
...
function enrichStaticData(symbol, market){
  const db = {
  ...
  return { ...market, ...(db[symbol] || { eps:null, pe:null, roe:null, institutionalReady:false, maReady:false }) };
}
...
function parseVolume(value) { const n = numberOrNull(value); return n === null ? null : n; }
...
function json(data) {
  return { statusCode: 200, headers: { "Content-Type": "application/json; charset=utf-8", "Access-Control-Allow-Origin": "*" }, body: JSON.stringify(data) };
}
function stockName(symbol) {
  const names = { "2330":"台積電","2317":"鴻海","2377":"微星","6214":"精誠","2881":"富邦金","2303":"聯電","6274":"台燿","3037":"欣興","0050":"元大台灣50","00918":"大華優利高填息30" };
...
function mockMarket(symbol) {
  ...
  const data = profiles[symbol] || { name:"台股個股", price:100, changePercent:0, volume:0 };
